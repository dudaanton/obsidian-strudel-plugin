...
import { Pattern, noteToMidi, freqToMidi, isPattern } from '../core/index.mjs'
...
Pattern.prototype.pianoroll = function (options = {}) {
  ...
  const inFrame = (hap, t) =>
    (!hideNegative || hap.whole.begin >= 0) && hap.isWithinTime(t + from, t + to)
  this.draw(
    (haps, time) => {
      __pianoroll({
        ...options,
...
export function __pianoroll({
  ...
  haps.forEach((event) => {
    ...
    if (labels) {
      const defaultLabel =
        event.value.note ?? event.value.s + (event.value.n ? `:${event.value.n}` : '')
