...
class ShapeProcessor extends AudioWorkletProcessor {
  ...
  process(inputs, outputs, parameters) {
    ...
    for (let n = 0; n < blockSize; n++) {
      for (let i = 0; i < input.length; i++) {
        output[i][n] =
          (((1 + shape) * input[i][n]) / (1 + shape * Math.abs(input[i][n]))) * postgain
      }
    }
    return true
  }
  ...
  process(_input, outputs, params) {
    ...
    for (let i = 0; i < output[0].length; i++) {
      ...
      for (let n = 0; n < voices; n++) {
        ...
        const freqVoice = applySemitoneDetuneToFrequency(
          freq,
          getUnisonDetune(voices, freqspread, n)
        )
...
class WavetableOscillatorProcessor extends AudioWorkletProcessor {
  ...
  _warpPhase(phase, amt, mode) {
    switch (mode) {
      ...
      case WarpMode.BENDMP: {
        return amt < 0.5
          ? this._warpPhase(phase, 1 - 2 * amt, 3)
          : this._warpPhase(phase, 2 * amt - 1, 2)
      }
      case WarpMode.SYNC: {
  ...
  _chooseMip(dphi) {
    ...
    let level = 0
    while (
      level + 1 < (this.tables?.length || 1) &&
      approxHarm < this.tables[level][0].length / 8
    ) {
      level++
    }
    return level
