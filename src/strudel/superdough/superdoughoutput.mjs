...
export class Orbit {
  ...
  constructor(audioContext) {
    ...
    this.summingNode = new GainNode(audioContext, {
      gain: 1,
      channelCount: 2,
      channelCountMode: 'explicit',
    })
  ...
  getDelay(delaytime = 0, feedback = 0.5, t) {
    ...
    this.delayNode.delayTime.value !== delaytime &&
      this.delayNode.delayTime.setValueAtTime(delaytime, t)
    this.delayNode.feedback.value !== feedback &&
      this.delayNode.feedback.setValueAtTime(feedback, t)
  ...
  getReverb(duration, fade, lp, dim, ir, irspeed, irbegin) {
    ...
    if (this.reverbNode == null) {
      this.reverbNode = this.audioContext.createReverb(
        duration,
        fade,
        lp,
        dim,
...
export class SuperdoughOutput {
  ...
  initializeAudio() {
    ...
    this.channelMerger = new ChannelMergerNode(audioContext, {
      numberOfInputs: audioContext.destination.channelCount,
    })
  ...
  connectToDestination = (input, channels = [0, 1]) => {
    ...
    channels.forEach((ch, i) => {
      splitter.connect(
        this.channelMerger,
        i % stereoMix.channelCount,
        ch % this.audioContext.destination.channelCount
      )
    })
  }
}
