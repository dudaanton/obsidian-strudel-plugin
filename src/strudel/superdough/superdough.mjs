...
function loadWorklets() {
  if (!workletsLoading) {
    ...
    workletsLoading = Promise.all(
      allWorkletURLs.map((workletURL) => audioCtx.audioWorklet.addModule(workletURL))
    )
  }
...
function getFilterType(ftype) {
  ...
  return typeof ftype === 'number'
    ? filterTypes[Math.floor(_mod(ftype, filterTypes.length))]
    : ftype
}
...
export function getAnalyzerData(type = 'time', id = 1) {
  ...
  if (!getter) {
    throw new Error(
      `getAnalyzerData: ${type} not supported. use one of ${Object.keys(getter).join(', ')}`
    )
  }
  getter()
...
export const superdough = async (value, t, hapDuration, cps = 0.5, cycle = 0.5) => {
  ...
  stretch !== undefined &&
    chain.push(getWorklet(ac, 'phase-vocoder-processor', { pitchFactor: stretch }))
  ...
  if (room > 0) {
    ...
    if (ir !== undefined) {
      ...
      if (Array.isArray(sample)) {
        ...
        url = Object.values(sample.data.samples).flat()[
          i % Object.values(sample.data.samples).length
        ]
      }
