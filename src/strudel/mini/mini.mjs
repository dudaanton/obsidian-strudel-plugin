...
import Fraction, { lcm } from '../core/fraction.mjs'
      ...
      switch (op.type_) {
        case 'stretch': {
          ...
          if (!legalTypes.includes(type)) {
            throw new Error(
              `mini: stretch: type must be one of ${legalTypes.join('|')} but got ${type}`
            )
          }
        ...
        case 'range': {
          ...
          let range = (apat, bpat) =>
            apat.squeezeBind((a) => bpat.bind((b) => strudel.fastcat(...arrayRange(a, b))))
      ...
      switch (alignment) {
        ...
        case 'polymeter': {
          ...
          const aligned = children.map((child) =>
            child.fast(stepsPerCycle.fmap((x) => x.div(child.__weight)))
          )
          pat = strudel.stack(...aligned)
          break
        }
        case 'rand': {
          pat = strudel.chooseInWith(
            strudel.rand.early(randOffset * ast.arguments_.seed).segment(1),
            children
          )
          if (with_steps.length) {
            pat._steps = lcm(...with_steps.map((x) => Fraction(x._steps)))
        ...
        default: {
          ...
          if (weightedChildren) {
            ...
            pat = strudel.timeCat(
              ...ast.source_.map((child, i) => [
                child.options_?.weight || strudel.Fraction(1),
                children[i],
              ])
            )
