...
export class SupersawOsc {
  ...
  update(freq) {
    ...
    for (let n = 0; n < this.voices; n++) {
      const freqAdjusted = applySemitoneDetuneToFrequency(
        freq,
        getUnisonDetune(this.voices, this.freqspread, n)
      )
...
export const getADSR = (params, curve = 'linear', defaultValues) => {
  ...
  const sustain =
    s != null ? s : (a != null && d == null) || (a == null && d == null) ? envmax : envmin
  return [
    Math.max(a ?? 0, envmin),
    Math.max(d ?? 0, envmin),
    Math.min(sustain, envmax),
    Math.max(r ?? 0, releaseMin),
  ]
}
...
export class DoughVoice {
  ...
  constructor(value) {
    ...
    if ($.penv) {
      ...
      ;[$.pattack, $.pdecay, $.psustain, $.prelease] = getADSR([
        $.pattack,
        $.pdecay,
        $.psustain,
        $.prelease,
    ...
    if ($.fmi) {
      ...
      if ($.fmenv) {
        ...
        ;[$.fmattack, $.fmdecay, $.fmsustain, $.fmrelease] = getADSR([
          $.fmattack,
          $.fmdecay,
          $.fmsustain,
          $.fmrelease,
    ...
    if ($.lpenv) {
      ...
      ;[$.lpattack, $.lpdecay, $.lpsustain, $.lprelease] = getADSR([
        $.lpattack,
        $.lpdecay,
        $.lpsustain,
        $.lprelease,
    ...
    if ($.hpenv) {
      ...
      ;[$.hpattack, $.hpdecay, $.hpsustain, $.hprelease] = getADSR([
        $.hpattack,
        $.hpdecay,
        $.hpsustain,
        $.hprelease,
    ...
    if ($.bpenv) {
      ...
      ;[$.bpattack, $.bpdecay, $.bpsustain, $.bprelease] = getADSR([
        $.bpattack,
        $.bpdecay,
        $.bpsustain,
        $.bprelease,
